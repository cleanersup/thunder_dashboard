...
export interface CommSummaryStepProps {
  costs:          CostBreakdown;
  total:          number;
...
export function CommSummaryStep({
  costs, total, serviceSubType, client,
...
}: CommSummaryStepProps) {
  const netProfit = total - costs.totalOperationCost;
    ...
    <div className="space-y-4">
      ...
      <Card style={{ borderColor: "hsl(var(--primary))" }} className="border-2">
        <CardContent className="p-4 text-center">
          ...
          <p className="text-xs text-muted-foreground mt-1">{serviceSubType}</p>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardContent className="p-4 space-y-3">
          ...
          {[
            ...
            { label: "Overhead",             val: costs.overheadCost },
          ].map(({ label, val }) => (
          ...
          <div className="border-t pt-2 flex justify-between text-sm font-semibold">
            ...
            <span>{fmt(costs.totalOperationCost)}</span>
          </div>
