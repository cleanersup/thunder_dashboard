...
export function EstimateDetailsModal({ open, onOpenChange, estimateId, onEdit, onOpenEstimateWizard }: EstimateDetailsModalProps) {
  ...
  // ── Load estimate when modal opens ────────────────────────────────────────
  useEffect(() => {
    if (open && estimateId) {
  ...
  function handleConvertToInvoice() {
    ...
    const dueDate = new Date(); dueDate.setDate(today.getDate() + 7);
  ...
  if (loading) {
    return (
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="max-w-[580px] h-[85vh] p-0 gap-0">
          <div className="flex items-center justify-center h-full">
            <p className="text-muted-foreground text-sm">Loading estimate...</p>
          </div>
        </DialogContent>
      </Dialog>
    );
  ...
  const statusBg    = getStatusBg(f.status);
  ...
  // Residential: rooms (numeric key-value pairs)
  ...
  // Residential: additional services (numeric)
  ...
  // Residential: extras (array of strings)
  const draftExtras: string[] = f.status === "Draft" && f.serviceType === "Residential"
    ? (Array.isArray(draftFd.extras) ? draftFd.extras as string[] : [])
    : [];

  // Commercial: key info rows
  const draftCommercialRows: [string, string][] = f.status === "Draft" && f.serviceType === "Commercial"
    ? ([
        ...
        .filter(([, v]) => v !== undefined && v !== null && v !== "" && v !== 0 && v !== "")
  ...
  return (
    <>
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="max-w-[580px] h-[85vh] p-0 gap-0 overflow-hidden bg-muted">

          {/* ── Header ─────────────────────────────────────────────────────── */}
          <div className="flex items-center gap-2 px-6 pr-12 py-2.5 border-b border-border/50 bg-sidebar">
            <h2 className="text-lg font-bold text-white">{f.estimateNumber}</h2>
            <Badge
            ...
            <div className="p-[5px]">
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-[5px]">
                ...
                <div className="space-y-[5px]">
                  {f.status === "Draft" ? (
                    <>
                      {/* Client Info — from actual client/lead */}
                      <Card className="border border-border/50">
                        <CardContent className="p-5">
                          {draftClientInfo ? (
                            <>
                              <h3 className="text-lg font-semibold mb-4">{draftClientInfo.name}</h3>
                      ...
                      {/* Service Details — with step progress */}
                      <Card className="border border-border/50">
                        <CardContent className="p-5">
                          ...
                          <CardContent className="p-5">
                            ...
                            <div className="grid grid-cols-2 gap-2">
                              {draftRooms.map(([label, value], i) => (
                                <div key={i} className="flex justify-between items-center p-2 bg-secondary/30 rounded-[5px]">
                          ...
                          <CardContent className="p-5">
                            ...
                            <div className="grid grid-cols-2 gap-2">
                              {draftExtras.map((item, i) => (
                                <div key={i} className="flex items-center gap-2 p-2 bg-secondary/30 rounded-[5px]">
                      ...
                      {f.serviceType === "Residential" && (
                        ...
                        <Card className="border border-border/50">
                          <CardContent className="p-5">
                            ...
                            <div className="grid grid-cols-2 gap-2">
                              {!!(draftFd.pets && draftFd.pets !== "No" && draftFd.pets !== "No pets") && (
                                <div className="p-2 bg-secondary/30 rounded-[5px]">
                      ...
                      {/* Commercial: Property & Service Info */}
                    ...
                    <>
                      ...
                      <Card className="border border-border/50">
                        <CardContent className="p-5">
                          <h3 className="text-lg font-semibold mb-4">{f.clientName}</h3>
                          <div className="space-y-3">
                            {f.companyName && (
                              <div className="flex items-center gap-3">
                      ...
                      {/* Rooms Breakdown */}
                      {f.serviceType === "Residential" && Object.keys(f.mainData).length > 0 && (
                        <Card className="border border-border/50">
                          <CardContent className="p-5">
                            <h4 className="text-sm font-semibold mb-3">Rooms Breakdown</h4>
                            <div className="grid grid-cols-2 gap-2">
                      ...
                      {/* Additional Items */}
                      {Object.keys(f.additionalData).length > 0 && (
                        <Card className="border border-border/50">
                          <CardContent className="p-5">
                ...
                <div className="space-y-[5px]">
                    ...
                    <CardContent className="p-5">
                      <div className="text-center">
                        ...
                        <p className="text-4xl font-bold" style={{ color: statusColor }}>
                          {f.status === "Draft" ? "—" : `$${f.total.toFixed(2)}`}
                        </p>
                      </div>
                      {f.status !== "Draft" && (
                        <div className="mt-4 p-3 rounded-lg flex justify-between items-center" style={{ backgroundColor: "hsl(var(--green-vibrant) / 0.05)" }}>
                          <div className="flex items-center gap-2">
                            <DollarSign className="h-5 w-5" style={{ color: "hsl(var(--green-vibrant))" }} />
                            <span className="font-semibold" style={{ color: "hsl(var(--green-vibrant))" }}>Net Profit</span>
                          </div>
                          <span className="text-xl font-bold" style={{ color: "hsl(var(--green-vibrant))" }}>
                  ...
                  {/* Cost Breakdown — hidden for drafts (placeholder data) */}
                  {f.status !== "Draft" && (
                    <Card className="border border-border/50">
                      <CardContent className="p-5">
                        <h4 className="text-sm font-semibold mb-4">Operation Cost Breakdown</h4>
                        <div className="space-y-3">
                          {[
                          ...
                          <div className="border-t border-border/50 pt-3 flex justify-between items-center">
                            ...
                            <span className="text-sm font-bold">${(estimate.total_operation_cost ?? f.total * 0.60).toFixed(2)}</span>
                          </div>
                        </div>
                      </CardContent>
                    </Card>
                  ...
                  {/* Extra Services */}
                  {Object.keys(f.extraServices).length > 0 && (
                    <Card className="border border-border/50">
                      <CardContent className="p-5">
                        <h4 className="text-sm font-semibold mb-3">Extra Services</h4>
                        <div className="grid grid-cols-2 gap-2">
                  ...
                  {f.discountType && f.discountValue && (
                    <Card className="border border-border/50">
                      <CardContent className="p-5">
                        ...
                        <div className="flex items-center justify-between p-3 bg-destructive/10 rounded-[5px]">
                        ...
                        {f.status === "Draft" ? (
                          <>
                            <Button variant="outline" size="sm" className="justify-start h-10"
                              ...
                              <Play className="w-4 h-4 mr-2 text-primary" /> Continue
                            </Button>
                            <Button variant="outline" size="sm" className="justify-start h-10"
                              onClick={handleStartFreshDraft}>
                              <RefreshCw className="w-4 h-4 mr-2 text-primary" /> Start Fresh
                            </Button>
                            <Button variant="outline" size="sm" className="justify-start h-10 text-destructive hover:text-destructive"
                              onClick={() => setIsDeleteDraftDialogOpen(true)}>
                          ...
                          <>
                            {f.status === "Pending" && (
                              <>
                                <Button variant="outline" size="sm" className="justify-start h-10 text-success hover:text-success"
                            ...
                            {f.status === "Accepted" && !isAlreadyInRoute && (
                              <Button variant="outline" size="sm" className="justify-start h-10"
                                onClick={() => navigate("/scheduling")}>
                                <Calendar className="w-4 h-4 mr-2 text-primary" /> Add to Route
                              </Button>
                            )}
                            <Button variant="outline" size="sm" className="justify-start h-10" onClick={handleEdit}>
                              <Edit className="w-4 h-4 mr-2 text-primary" /> Edit
                            </Button>
                            <Button variant="outline" size="sm" className="justify-start h-10"
                              onClick={handleShare} disabled={isGeneratingLink}>
                              <Share className="w-4 h-4 mr-2 text-primary" />
                            ...
                            <Button variant="outline" size="sm" className="justify-start h-10" onClick={handleConvertToInvoice}>
                              <FileText className="w-4 h-4 mr-2 text-primary" /> Convert to Invoice
                            </Button>
                            {f.status !== "Canceled" && (
                              <Button variant="outline" size="sm" className="justify-start h-10 text-destructive hover:text-destructive"
